...
<!-- @index Formatting; Style specifications; CSS needs -->
The index is actually formatted as a bulleted list (the `ul` tag in HTML). 
This facilitates 
appropriate indentation of entries and subentries. But if actual bullets 
are present, they are unattractive. Additionally, sometimes linked text formatted 
as code does get colored the same way as other text. The `write_vi()` function
writes appropriate HTML styling to the file in an attempt to fix these issues,
and you will see that code at the beginning of the generated index. If you need
something different there, you can specify it as the `style` argument in 
`vigindex()` (or in `write_vi()`). For some reason, this works fine
with `rmarkdown::html_document` but not with `rmarkdown::html_vignette`.
